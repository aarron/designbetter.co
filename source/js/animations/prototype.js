...
var init = function() {
  canvas = document.getElementById("prototype-animation");
  if (canvas) {
    handleComplete();
  }
}
function handleComplete() {
  var container = $('#prototype-animation-container');
  //This function is always called, irrespective of the content. You can use the variable "stage" after it is created in token create_stage.
  exportRoot = new prototypeLib.Remote4();
  stage = new createjs.Stage(canvas);
  stage.addChild(exportRoot);
  //Registers the "tick" event listener.
  createjs.Ticker.setFPS(prototypeLib.properties.fps);
  ...
  (function(isResp, respDim, isScale, scaleType) {
    ...
    function resizeCanvas() {
      var w = prototypeLib.properties.width, h = prototypeLib.properties.height;
