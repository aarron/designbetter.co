...
var init = function() {
  canvas = document.getElementById("show-and-tell-animation");
  if (canvas) {
    handleComplete();
  }
}
function handleComplete() {
  var container = $('#show-and-tell-animation-container');
  //This function is always called, irrespective of the content. You can use the variable "stage" after it is created in token create_stage.
  exportRoot = new showAndTellLib.hamster();
  stage = new createjs.Stage(canvas);
  stage.addChild(exportRoot);
  //Registers the "tick" event listener.
  createjs.Ticker.setFPS(showAndTellLib.properties.fps);
  ...
  (function(isResp, respDim, isScale, scaleType) {
    ...
    function resizeCanvas() {
      var w = showAndTellLib.properties.width, h = showAndTellLib.properties.height;
