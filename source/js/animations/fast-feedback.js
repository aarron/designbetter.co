...
var init = function() {
  canvas = document.getElementById("fast-feedback-animation");
  if (canvas) {
    handleComplete();
  }
}
function handleComplete() {
  var container = $('#fast-feedback-animation-container');
  //This function is always called, irrespective of the content. You can use the variable "stage" after it is created in token create_stage.
  exportRoot = new fastFeedbackLib.Chef2();
  stage = new createjs.Stage(canvas);
  stage.addChild(exportRoot);
  //Registers the "tick" event listener.
  createjs.Ticker.setFPS(fastFeedbackLib.properties.fps);
  ...
  (function(isResp, respDim, isScale, scaleType) {
    ...
    function resizeCanvas() {
      var w = fastFeedbackLib.properties.width, h = fastFeedbackLib.properties.height;
