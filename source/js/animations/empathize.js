...
var init = function() {
  canvas = document.getElementById("empathize-animation");
  if (canvas) {
    handleComplete();
  }
}
function handleComplete() {
  var container = $('#empathize-animation-container');
  //This function is always called, irrespective of the content. You can use the variable "stage" after it is created in token create_stage.
  exportRoot = new empathizeLib.Astronaut2();
  stage = new createjs.Stage(canvas);
  stage.addChild(exportRoot);
  //Registers the "tick" event listener.
  createjs.Ticker.setFPS(empathizeLib.properties.fps);
  ...
  (function(isResp, respDim, isScale, scaleType) {
    ...
    function resizeCanvas() {
      var w = empathizeLib.properties.width, h = empathizeLib.properties.height;
