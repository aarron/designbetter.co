...
var init = function() {
  canvas = document.getElementById("ideate-animation");
  if (canvas) {
    handleComplete();
  }
}
function handleComplete() {
  var container = $('#ideate-animation-container');
  //This function is always called, irrespective of the content. You can use the variable "stage" after it is created in token create_stage.
  exportRoot = new ideateLib.Lunar_Lander4();
  stage = new createjs.Stage(canvas);
  stage.addChild(exportRoot);
  //Registers the "tick" event listener.
  createjs.Ticker.setFPS(ideateLib.properties.fps);
  ...
  (function(isResp, respDim, isScale, scaleType) {
    ...
    function resizeCanvas() {
      var w = ideateLib.properties.width, h = ideateLib.properties.height;
